...
export default async function CalendarPage({ searchParams }: { searchParams: Promise<{ open?: string }> }) {
	...
	const shareUrl = `${process.env.NEXT_PUBLIC_URL || 'https://7herchu.vercel.app'}/share/speaker?openExternalBrowser=1`
										...
										<CardContent className="p-4">
											<div className="flex justify-between">
												<div>
													...
													{/* 行2（僅手機）：日期 */}
													<div className="sm:hidden text-sm text-gray-600 flex items-center gap-2 -mt-1 mb-1">
														<CalendarIcon className="w-4 h-4" />
														<span>{format(e.startAt, 'MM/dd（EEEEE）', { locale: zhTW })}</span>
													</div>
